//------------------------------------------------------------
// 3DPhysics_2_4_Pre.cpp
// 3D포물선운동(곡사포, 고저차 있음)의 기점 소스
// 
//------------------------------------------------------------
...
#define SHELL_FIRST_VEL				0.15f				// 포탄초속
...
MY_PLAYER	Player_1;							// 플레이어데이터
MY_SHELL	Shell_1;							// 포탄데이터
XMFLOAT3	v3TargetPos;						// 타깃위치


// 발사각 계산(곡사, 높이차도 있는 경우 이곳을 구현)
// fDistance_XZ:xz평면상에서의 표적과 발사점의 거리, fDistance_Y:표적과 발사점의 고저차
// fVel_0:포탄의 초속
float GetBarrelAngle( float fDistance_XZ, float fDistance_Y, float fVel_0 )
{
...
int InitTatget( void )									// 타깃 초기화
{
	v3TargetPos.x = ( float )rand() * 10.0f / RAND_MAX - 5.0f;
	v3TargetPos.y = ( float )rand() * 3.0f / RAND_MAX + SHELL_RADIUS;
...
int MoveShell( void )
{
	if ( Shell_1.bActive ) {
		...
		Shell_1.v3Vel.y -= GR;
		if ( ( Shell_1.v3Pos.y < v3TargetPos.y ) && ( Shell_1.v3Vel.y < 0.0f ) ) {
			Shell_1.bActive = false;
			InitTatget();
		}
	}
...
{
	float				fDistance_XZ, fDistance_Y;		// 거리
	// 포발사
	if ( !( Shell_1.bActive ) ) {
		Player_1.fPhi = -atan2f( v3TargetPos.z, v3TargetPos.x );
		fDistance_XZ = sqrtf( v3TargetPos.x * v3TargetPos.x + v3TargetPos.z * v3TargetPos.z );
		fDistance_Y = v3TargetPos.y;
		Player_1.fTheta = GetBarrelAngle( fDistance_XZ, fDistance_Y, SHELL_FIRST_VEL );
		Shell_1.bActive = true;
		Shell_1.v3Pos = XMFLOAT3( 0.0f, 0.0f, 0.0f );
...
XMMATRIX CreateShadowMatrix( XMFLOAT3 v3Light, float fGround_y )	// 그림자 행렬 생성
{
	...
	matShadowed._42 = fGround_y + 0.01f;			// 지면에서 뜨게 미세조정
...
int MakeSphereIndexed( float x, float y, float z, float r,
					   ...
					   int nIndexOffset )			// 구 만들기(중심 위치와 인덱스)
{
	int					i, j;
	float				fTheta;
	float				fPhi;
...
ID3D11DepthStencilState *g_pDSDepthState_NoWrite = NULL;	// Z버퍼의 스테이트(Z버퍼쓰기 금지)
...
int WINAPI _tWinMain( HINSTANCE hInst, HINSTANCE, LPTSTR, int )
{
    ...
    g_hWnd = CreateWindow( _T( "D3D Sample" ), _T( "3DPhysics_2_4(Secret:고저차도 있는 경우의 표적사격, 기점 소스)" ),
