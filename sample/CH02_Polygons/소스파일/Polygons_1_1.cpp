...
int RenderScanLine( void )
{
	...
	if ( a12 < a13 ) {
		for ( y = ( int )y1; y < ( int )y2; y++ ) {		// y방향 루프
				for ( x = ( int )xl; x < ( int )xr; x++ ) {	// x방향 루프
					DrawPoints( x, y, 255, 255, 255 );
				}
			xl += a12;  xr += a13;
			FlushDrawingPictures();
		}
		for ( y = ( int )y2; y < ( int )y3; y++ ) {		// y방향 루프
				for ( x = ( int )xl; x < ( int )xr; x++ ) {	// x방향 루프
					DrawPoints( x, y, 255, 255, 255 );
				}
			xl += a23;  xr += a13;
...
#define MAX_BUFFER_VERTEX				20000	// 최대 버퍼 정점수
...
int Cleanup( void )
{
    ...
    SAFE_RELEASE( g_pImmediateContext );					// 디바이스 컨텐스트
