...
#define VIEW_WIDTH			800					// 화면 너비
#define VIEW_HEIGHT			600					// 화면 높이
#define POLY_SPEED			0.1f				// 폴리곤 속도
...
{
	float		fAngle;
	XMMATRIX		xmResult;							// 결과행렬
	static float	x = 0.0f;							// x 좌표

	fAngle = 2.0f * 3.1415f * ( float )( timeGetTime() % 2000 ) / 2000.0f;
	xmResult = XMMatrixRotationY( fAngle );
	if ( GetAsyncKeyState( VK_LEFT  ) ) x -= POLY_SPEED;	// 좌
	if ( GetAsyncKeyState( VK_RIGHT ) ) x += POLY_SPEED;	// 우
	xmResult._41 = x;
	return xmResult;
}
...
#define MAX_BUFFER_VERTEX				1000	// 최대 버퍼 정점수
...
int Draw3DPolygon( float x1, float y1, float z1, int nColor1,
				   ...
				   float x3, float y3, float z3, int nColor3 )
{
	if ( g_nVertexNum > ( MAX_BUFFER_VERTEX - 3 ) ) return -1;	// 정점이 버퍼로부터 넘칠 경우는 그리지 않는다
