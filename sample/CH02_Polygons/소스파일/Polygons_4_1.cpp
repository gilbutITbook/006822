...
// 극좌표에서 직교좌표로 변환
// 지구의 위도, 경도와 기본적으로 같은 각도를 가진다
// 위도 θ= fTheta, 경도φ=fPhi
...
int DrawChangingPictures( void )				// 그리기 
{
	int					i;
	float				fTheta1, fTheta2;
	float				fPhi1, fPhi2;
	float				fAngleDelta;
	XMFLOAT3			p1, p2, p3, p4;				// 4점의 좌표
	...
	for ( i = 0; i < CORNER_NUM / 2; i++ ) {
		p1 = ConvertRTPToXYZ( R, fTheta1, fPhi1 );	// 정점1
		p2 = ConvertRTPToXYZ( R, fTheta1, fPhi2 );	// 정점2
		p3 = ConvertRTPToXYZ( R, fTheta2, fPhi1 );	// 정점3
		p4 = ConvertRTPToXYZ( R, fTheta2, fPhi2 );	// 정점4
...
int Draw3DPolygon( float x1, float y1, float z1, int nColor1,
				   ...
				   float x3, float y3, float z3, int nColor3 )
{
	if ( g_nVertexNum > ( MAX_BUFFER_VERTEX - 3 ) ) return -1;	// 정점이 버퍼에서 넘칠 때는 그리지 않고
