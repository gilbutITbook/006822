...
#define GROUND_DIVIDE_NUM			30					// 바닥분할수
#define BLOCK_NUM				( GROUND_DIVIDE_NUM )	// 지형블록수
#define BLOCK_WIDTH				( GROUND_SIZE / BLOCK_NUM )		// 블록의 너비
...
ID3D11DepthStencilState *g_pDSDepthState_NoWrite = NULL;	// Z버퍼의스테이트(Z버퍼쓰기금지)
...
ID3D11InputLayout		*g_pInputLayout;		// 셰이더입력레이아웃ト
...
int WINAPI _tWinMain( HINSTANCE hInst, HINSTANCE, LPTSTR, int )
{
    ...
    if( SUCCEEDED( InitD3D() ) && SUCCEEDED( MakeShaders() ) )
    {
        ...
        {
			if ( SUCCEEDED( InitGeometry() ) ) {					// 지오메트리作成
				InitPlayer();										// 플레이어の초기화
				// Show the window
