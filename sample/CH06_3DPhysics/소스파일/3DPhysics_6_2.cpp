...
#define PLAYER_SPEED				0.2f				// 플레이어 이동속도
#define GROUND_SIZE					20.0f				// 바닥 크기
#define GROUND_DIVIDE_NUM			50					// 바닥 분할수
#define BLOCK_NUM				( GROUND_DIVIDE_NUM )	// 지형블록수
#define BLOCK_WIDTH				( GROUND_SIZE / BLOCK_NUM )		// 블록 너비
#define ROUND_TIME					2000				// 주기(밀리 초 단위)
#define WAVE_NUM					16					// 파의 수
#define WAVE_HEIGHT					0.066f				// 파의 높이
#define WAVE_VELOCITY				0.003f				// 파의 속도
...
int InitWaves( void )
{
	...
	for ( i = 0; i < WAVE_NUM; i++ ) {
		Waves[i].fLength = rand() * 15.0f / RAND_MAX + 1.5f;	// 파장
		Waves[i].fAmp = WAVE_HEIGHT * Waves[i].fLength;			// 진폭	
		fAngle = 2.0f * PI * rand() / RAND_MAX;
		Waves[i].kx = cosf( fAngle ) / Waves[i].fLength;		// 파수벡터
		Waves[i].kz = sinf( fAngle ) / Waves[i].fLength;
		Waves[i].fDelta = 2.0f * PI * rand() / RAND_MAX;		// 위상
	}
