...
XMMATRIX CreateWorldMatrix( void )
{
	...
	return matRot_Y * matRot_X;						// 회전 합성
}
...
{
	int					i, j;
	CUSTOMVERTEX		Vertices[( CORNER_NUM + 1 ) * ( CORNER_NUM / 2 + 1 )];	// 정점 데이터
...
{
    XMMATRIX	matView;					// 변환행렬
    XMMATRIX	matWorld;					// 월드행렬
	XMFLOAT3	v3Light;					// 광원방향 벡터
	float		fAmbient;					// 주변광 세기
	float		fDirectional;				// 직사광 세기
};
    ...
    if( SUCCEEDED( InitD3D() ) && SUCCEEDED( MakeShaders() ) )
    {
        ...
        if( SUCCEEDED( InitDrawModes() ) )
        {
			if ( SUCCEEDED( InitGeometry() ) ) {					// 지오메트리 작성
				...
				QueryPerformanceCounter( &nLastTime );				// 1프레임전 시각 초기회
