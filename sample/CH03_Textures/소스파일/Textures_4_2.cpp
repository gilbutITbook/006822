...
#define TRANS_SPEED			0.3f				// 평행이동속도
#define R					2.0f				// 도형의 크기
#define CORNER_NUM			20					// 각수
#define CYLINDER_LENGTH		20.0f				// 원기둥의 길이
...
{
	static float	fAngleY = 0.0f;						// x축 회전각
	static float	fEffectY = 0.0f;					// 효과의 y좌표
...
{
	int					i, j;
	CUSTOMVERTEX		Vertices[( CORNER_NUM + 1 ) * ( CORNER_NUM + 1 )];	// 정점 데이터
	WORD				wIndices[CORNER_NUM * CORNER_NUM * 2 * 3];	//인덱스 데이터
	float				fTheta;
	float				fAngleDelta;
	int					nIndex;						// 데이터의 인덱스
	int					nIndexY;					// y방향 인덱스
	...
	// 정점 데이터 작성 
	vs = ( timeGetTime() % 500 ) / 500.0f;			// 시간요소
