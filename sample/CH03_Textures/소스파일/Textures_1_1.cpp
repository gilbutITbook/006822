...
// 텍스처 매핑을 한다
// 텍스처를 붙이기 위해 각 폴리곤에
// 어떻게 텍스처를 붙일 것인지 확정하는
// uv좌표를 지정할 필요가 있다
// uv좌표는 하나하나의 정점에 대해서 그 정점이
// 이미지 안의 어느 부분에 해당하는지 확정함으로써
// 이미지를 붙이는 방법을 유일하게 결정한다
// 단, uv좌표의 좌푯값은 도트단위가 아니라는 점에 주의하자
// uv좌표는 텍스처의 크기에 관계없이
// 텍스처의 끝에서 끝까지가 0～1이 되도록 좌표로 지정한다
// 그렇게 함으로써 실제 텍스처의 해상도에 관계없이
// 똑같은 방식으로 텍스처를 매핑할 수 있게 된다
...
ID3D11DeviceContext		*g_pImmediateContext;	// 디바이스 컨텐스트
...
ID3D11VertexShader		*g_pVertexShader;		// 정점 셰이더(버텍스 셰이더)
...
// 텍스처는 최종적으로 픽셀 셰이더에서
// 실제로 어떻게 붙여질지가 결정되므로 
// 픽셀 셰이더에 붙일 텍스처의 이미지를
// 넘겨줄 필요가 있다
// 픽셀 셰이더는 이 함수에 전달된 이미지와
// 버텍스 셰이더로부터 들어온 uv좌표로
// 어느 텍셀을 어떻게 샘플링할지 결정한다
