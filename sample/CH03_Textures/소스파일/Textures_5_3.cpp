...
#define MAX_EXPLOSION_NUM	10					// 최대폭발수
#define EXPLOSION_INTERVAL	200					// 폭발 발생 간격(밀리초 단위)
#define EXPLOSION_AREA		4.0f				// 폭발 발생 범위
#define EXPLOSION_SPEED		0.05f				// 폭발 확대 속도
#define EXPLOSION_R1		1.0f				// 밝기가 떨어지기시작하는 반지름
#define EXPLOSION_R2		2.0f				// 감소하는 반지름
...
struct Explosion {
	int			bActive;						// 유효 플래그
	int			nTime;							// 발생하고 나서의 시간
	XMFLOAT3	v3CenterPos;					// 중심위치
	float		fRadius;						// 반지름
	float		fAngle;							// 각도
	XMMATRIX	matMatrix;						// 변환행렬
	float		fBright;						// 밝기
};


Explosion		exExplode[MAX_EXPLOSION_NUM];	// 폭발 버퍼


int MakeSphereIndexed( CUSTOMVERTEX *pVertices, int *pVertexNum,
					   WORD *pIndices, int *pIndexNum )		// 구 작성(인덱스)
{
	int					i, j;
	float				fTheta;
	float				fPhi;
...
{
	int					i;
	int					nNowTime;				// 현재 시각(폭발간격 기반)
	static int			nLastTime = 0;			// 이전 시각(폭발간격 기반)
	float				x, z;					// x, z좌표
...
// 글로벌 변수
UINT  g_nClientWidth;							// 그리는 영역 너비
UINT  g_nClientHeight;							// 그리는 영역 높이
...
//    g_pImmediateContext->OMSetBlendState( NULL, NULL, 0xFFFFFFFF );		// 폭발 제어
	for ( i = 0; i < MAX_EXPLOSION_NUM; i++ ) {
		if ( exExplode[i].bActive ) {
