					  ...
					  int *pnRed, int *pnGreen, int *pnBlue )
{
	...
	GetBMPPixel( ( int )x,     ( int )y,     pmbSrc,
				 &nRed1, &nGreen1, &nBlue1 );			// 점1획득
	GetBMPPixel( ( int )x + 1, ( int )y,     pmbSrc,
				 &nRed2, &nGreen2, &nBlue2 );			// 점2획득
	GetBMPPixel( ( int )x,     ( int )y + 1, pmbSrc,
				 &nRed3, &nGreen3, &nBlue3 );			// 점3획득
	GetBMPPixel( ( int )x + 1, ( int )y + 1, pmbSrc,
				 &nRed4, &nGreen4, &nBlue4 );			// 점4획득
...
{
	int					x, y;
	float				bx, by;						// 로컬 베이스 좌표
	float				cx, cy;						// 로컬 좌표
	float				vBase1_x, vBase1_y;			// 기저벡터1
	float				vBase2_x, vBase2_y;			// 기저벡터2
	int					nRed, nGreen, nBlue;		// 픽셀색
	float				fAngle = PI / 6.0f;			// 회전각 

	vBase1_x = 0.05f * cosf( fAngle );						// 기저벡터1
	vBase1_y = 0.05f * sinf( fAngle );
	vBase2_x = 0.05f * cosf( fAngle + PI / 2.0f );			// 기저벡터2
	...
	for ( y = 0; y < VIEW_HEIGHT; y++ ) {		// y방향루프
		cx = bx; 
		cy = by; 
		for ( x = 0; x < VIEW_WIDTH; x++ ) {	// x방향루프
			GetBMPPixel( ( int )( cx + VIEW_WIDTH  / 2.0f ),
						 ( int )( cy + VIEW_HEIGHT / 4.0f ),
						 &mbPicture,
						 &nRed, &nGreen, &nBlue );		// 점 획득
...
{
	FILE				*fp;						// 파일 포인터
	unsigned char		*pcBMPBuf;					// BMP 데이터버퍼
...
//	if ( ( fp = fopen( szFileName, "rb" ) ) == NULL ) {
	...
	// 팔래트 데이터수 결정
	if ( pmbDest->pbiInfoH->biBitCount <= 8 ) {			// 256색 이하라면 팔래트 데이터가 있다
		if ( pmbDest->pbiInfoH->biClrUsed == 0 ) {		// 디폴트의 경우
...
int ReleaseBMPData( MemoryBMP *pmbRelease )			// BMP 데이터 해제
{
	if ( pmbRelease->pbfFileH ) {					// 메모리가 남아 있는가
