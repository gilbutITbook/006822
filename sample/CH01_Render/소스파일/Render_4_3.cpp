...
#define SCALE_MIN			0.1f				// 최소확대율
#define SCALE_MAX			2.0f				// 최대확대율
...
int RenderScanLine( void )
{
	...
	static float		fScale = 1.0f;			// 확대율
	static float		fScaleVel = 0.1f;

	for ( y = 0; y < VIEW_HEIGHT; y++ ) {		// y방향 루프
		for ( x = 0; x < VIEW_WIDTH; x++ ) {	// x방향 루프
			GetBMPPixel( x / fScale, y / fScale, &mbPicture,
						 &nRed, &nGreen, &nBlue );		// 점 취득
