...
#define HEIGHT_NUM					( GROUND_DIVIDE_NUM + 1	)	// 높이 데이터 수
#define BLOCK_NUM				( GROUND_DIVIDE_NUM )	// 지형 블록 수
...
XMMATRIX CreateWorldMatrix( float x, float y, float z, float fSize )	// 월드 행렬 생성
{
...
D3D_FEATURE_LEVEL       g_FeatureLevel;			// 피처레벨フィーチャーレベル
    ...
    if( SUCCEEDED( InitD3D() ) && SUCCEEDED( MakeShaders() ) )
    {
        ...
        if( SUCCEEDED( InitDrawModes() ) )
        {
			if ( SUCCEEDED( InitGeometry() ) ) {					// 지오메트리 작성
				...
				QueryPerformanceCounter( &nLastTime );				// 1프레임전시각 초기화
