...
#define R1					1.0f				// 구1의 크기
#define R2					0.5f				// 구2의 크기
#define CORNER_NUM			20					// 각수
#define SPHERE_SPEED		0.1f				// 구의 이동속도
...
Sphere		Sphere_1, Sphere_2;					// 구 데이터


// 구의 충돌판정
int CheckHit( Sphere *pCheckSphere1, Sphere *pCheckSphere2 )
{
	float				dx, dy, dz;						// x,y,z 좌표의 차분
	float				fDistanceSq;					// 구와 구 사이의 거리의 제곱
	int					nResult;						// 결과
...
int MakeSphereIndexed( CUSTOMVERTEX *pVertices, int *pVertexNum,
					   WORD *pIndices, int *pIndexNum )		// 구 만들기(인덱스된)
{
	int					i, j;
	float				fTheta;
	float				fPhi;
