//------------------------------------------------------------
// 3DCheckHit_3_2.cpp
// 평면적 삼각형과의 충돌판정(복수 삼각형 진입금지, 날뜀이 적게)
// 
//------------------------------------------------------------
...
#define CHECK_POLYGON_ANGLES		30					// 충돌판정 다각형의 각수
...
float Dot( XMFLOAT3 *pv3Vec1, XMFLOAT3 *pv3Vec2 )						// 벡터의 내적
{
	return pv3Vec1->x * pv3Vec2->x + pv3Vec1->y * pv3Vec2->y + pv3Vec1->z * pv3Vec2->z;
}


int CheckHit( XMFLOAT3 *pv3Point )							// 다각형(제한있는)과 점의 충돌판정
{
	int				i;
	float			fAngle;
	int				nAngleIndex;
...
#define MAX_MODEL_NUM					100		// 최대 모델 수 
...
ID3D11Device			*g_pd3dDevice;			// 다버아스
IDXGISwapChain			*g_pSwapChain;			// DXGI스왑체인
ID3D11DeviceContext		*g_pImmediateContext;	// 다비이스 컨텍스트
...
ID3D11DepthStencilState *g_pDSDepthState_NoWrite = NULL;	// Z버퍼의 스테이트(Z버퍼 쓰기금지)
