...
// 높이 데이터
// 지형의 네 모서리의 표고를 지정한다
// 단, 진짜로 지형의 네 모서리의 표고를 지정해버리면 
// 한장의 평면이 네 모서리를 지날 수 있다는 보증이 없어지므로(한번에 평면이 지날 수 있는 점이 일반적으로 3개까지이므로)
// 마지막 데이터는 무효이고 프로그램으로 표고가 계산된다
...
ID3D11DepthStencilState *g_pDSDepthState_NoWrite = NULL;	// Z버퍼의 스테이트(Z버퍼의 쓰기 없음)
