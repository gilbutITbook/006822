...
XMMATRIX MakeBillboardMatrix( XMMATRIX *pmatView )		// 빌보드 행렬 생성(원점한정)
{
...
#define MAX_BUFFER_INDEX				20000	// 최대인덱스수
...
HRESULT Render( void )
{
	...
	// 빌보드
    g_pImmediateContext->OMSetBlendState( g_pbsAddBlend, NULL, 0xFFFFFFFF );	// 가산 블렌드
	g_mmBillboard.mMatrix = MakeBillboardMatrix( &mView );			// 빌보드행렬 작성
...
{
	LARGE_INTEGER			nNowTime, nLastTime;		// 현재와 직전시작
