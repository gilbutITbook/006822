...
#define R							4.0f				// 도형 크기
#define CORNER_NUM					8					// 각수
#define LINE_NUM					6					// 라인수
...
// 방향 제어 행렬 작성
// 빌보드와 같은 판자 폴리곤뿐만 아니라 입체물을 임의의 방향으로 
// 향하게 할 수 있다. 
// 시점・종점을 받아서, 시점에서 종점으로 향하는 벡터의 방향으로 
// 원 x축 방향을 합성한다. 원 z축은 시점에서 종점으로 향하는 벡터와
// 가로방향 벡터(1,0,0)의 양쪽에 수직인 벡터의 방향으로 합성한다.
// 원 y축은변환 후 x축과 변환 후 z축 양쪽에 수직인 방향으로 향한다.
XMMATRIX MakeDirectMatrix( XMFLOAT3 *pv3Start, XMFLOAT3 *pv3End )
{
	...
	v3Up = Normalize( &v3Up );
	matArrowWorld = XMMatrixIdentity();				// 단위행렬로
