...
#define GROUND_BASE					0.0f				// 바닥의 기저 높이
#define GROUND_SIZE					20.0f				// 바닥 크기
#define GROUND_DIVIDE_NUM			50					// 바닥 분할수
#define HILL_HEIGHT					2.0f				// 언덕 높이
#define BLOCK_NUM				( GROUND_DIVIDE_NUM )	// 지형 블록수
#define BLOCK_WIDTH				( GROUND_SIZE / BLOCK_NUM )		// 블록너비
#define BILLBOARD_SIZE				2.0f				// 빌보드 크기
#define BILLBOARD_NUM				50					// 빌보드수
#define BILLBOARD_AREA				10.0f				// 빌보드가 배치되는 범위
...
MY_BILLBOARD	Billboards[BILLBOARD_NUM];		// 빌보드 데이터
				   ...
				   int nDataNum, XMMATRIX *pmView )
{
	...
	for ( i = 0; i < nDataNum; i++ ) {
		pSource[i].fSortKey = pmView->_13 * pSource[i].v3Pos.x +
							  ...
							  pmView->_33 * pSource[i].v3Pos.z;		// View좌표에서의 z성분
	}
	for ( i = 0; i < nDataNum; i++ ) {
		for ( j = i - 1; j >= 0; j-- ) {
...
int InitBillboard( void )								// 빌보드 초기화
{
	int				i;
	float			x, y, z;
...
ID3D11DeviceContext		*g_pImmediateContext;	// 디바이스 컨텍스트ト
...
ID3D11BlendState		*g_pbsAlphaBlend;		// 가산 블렌드
...
HRESULT Render( void )
{
	...
	// 빌보드
	g_pImmediateContext->OMSetDepthStencilState( g_pDSDepthState_NoWrite, 1 );	// Z버퍼에 쓰기 금지
    g_pImmediateContext->OMSetBlendState( g_pbsAlphaBlend, NULL, 0xFFFFFFFF );	// 가산 블렌딩
	...
	for ( i = 0; i < BILLBOARD_NUM; i++ ) {
		...
		g_mmBillboard.mMatrix = MakeBillboardMatrix( &mView, &( pSort[i]->v3Pos ) );	// 빌보드 행렬 작성
