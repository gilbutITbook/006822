...
HRESULT InitGeometry( void )
{
	...
	for ( i = 0; i < GROUND_DIVIDE_NUM + 1; i++ ) {
		fgets( szLineBuf, 200, fp );					// １라인읽어들이기
		pBufLoc = szLineBuf;
		x = -GROUND_SIZE / 2;
		v = ( float )i;
...
int Cleanup( void )
{
    ...
    SAFE_RELEASE( g_pDSDepthState );						// Z버퍼이 스테이트
