...
#define GROUND_BASE					-5.0f				// 바닥의 기저 높이
...
ID3D11DeviceContext		*g_pImmediateContext;	// 텍스트 컨텍스트
...
HRESULT Render( void )
{
	...
	// 빌보드
	g_pImmediateContext->OMSetDepthStencilState( g_pDSDepthState_NoWrite, 1 );	// Z버퍼에 쓰지 않음
	...
	for ( i = 0; i < BILLBOARD_NUM; i++ ) {
		...
		g_mmBillboard.mMatrix = MakeBillboardMatrix( &mView, &v3Pos );			// 빌보드 행렬작성
