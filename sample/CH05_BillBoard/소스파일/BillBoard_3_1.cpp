...
#define BILLBOARD_NUM				50					// 빌보드 수
#define BILLBOARD_AREA				10.0f				// 빌보드가 놓이는 범위
...
ID3D11BlendState		*g_pbsAlphaBlend;			// α블렌드
...
HRESULT Render( void )
{
	...
	// 빌보드
	int			i;
	g_pImmediateContext->OMSetDepthStencilState( g_pDSDepthState_NoWrite, 1 );	// Z버퍼쓰기금지
    g_pImmediateContext->OMSetBlendState( g_pbsAlphaBlend, NULL, 0xFFFFFFFF );	// α블렌드
	for ( i = 0; i < BILLBOARD_NUM; i++ ) {
		g_mmBillboard.v4AddColor = XMFLOAT4( Billboards[i].fMarkBright, 0.0f, 0.0f, 0.0f );
		g_mmBillboard.mMatrix = MakeBillboardMatrix( &mView, &( Billboards[i].v3Pos ) );	// 빌보드 행렬 작성
