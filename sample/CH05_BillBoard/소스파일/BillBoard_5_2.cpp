...
#define PLAYER_SPEED				0.15f				// 플레이어 이동속도
#define PLAYER_ANG_SPEED			( PI / 100.0f )		// 플레이어 회전속도
#define GROUND_SIZE					30.0f				// 바닥 크기
#define GROUND_DIVIDE_NUM			23					// 바닥 분할수
...
int				g_nHitTriangleIndex;						// 충돌삼각형 인덱스
...
XMFLOAT3 Add( XMFLOAT3 *pv3Vec1, XMFLOAT3 *pv3Vec2 )						// 벡터의 덧셈
{
	return XMFLOAT3( pv3Vec1->x + pv3Vec2->x,
					 pv3Vec1->y + pv3Vec2->y,
					 pv3Vec1->z + pv3Vec2->z );
...
// 지면 정점의 법선 벡터 작성
// 법선벡터를 보간하기 위해서는 각 정점에 대해서 법선벡터가 사전에 필요
...
int Make3DRectangleIndexed( float fWidth, float fHeight, float fDepth, int bCenterMode,
							...
							int nIndexOffset )				// 직육명체 작성(인덱스된)
{
	int					i;
	int					nIndex;						// 데이터의 인덱스
	int					nUpIndex2;					// 윗면 2번째 인덱스
...
UINT  g_nClientHeight;							// ㄱ릴 영역의 높이
...
HRESULT InitGeometry( void )
{
	...
	for ( i = 0; i < GROUND_DIVIDE_NUM + 1; i++ ) {
		...
		for ( j = 0; j < GROUND_DIVIDE_NUM + 1; j++ ) {
			y = ( float )atof( pBufLoc ) / 30.0f;				// 수치로 변환
			g_fHeights[i][j] = y;
    ...
    if( SUCCEEDED( InitD3D() ) && SUCCEEDED( MakeShaders() ) )
    {
        ...
        if( SUCCEEDED( InitDrawModes() ) )
        {
			if ( SUCCEEDED( InitGeometry() ) ) {					// 지오메트리 작성
				...
				QueryPerformanceCounter( &nLastTime );				// 1프레임전 시각초기화
